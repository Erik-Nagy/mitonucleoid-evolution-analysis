...
```r
install.packages(c("readxl", "readr", "dplyr", "tidyr", "stringr", "httr", "bio3d",
                   "ggplot2", "ggpubr",
                   "shiny", "r3dmol", "BiocManager"))
...
| 10 | `extract_all_sty_data.py` | Disorder (Metapredict), pLDDT, and SASA for all S/T/Y residues in each protein (background for pSTY vs STY plots) | `data/all_sty_data.tsv` |
...
Static figures are produced by three R scripts in `data_analysis/data_analysis_visualization/`.

**Run from RStudio** — open `run_all.R` and click *Source*, or from the command line:

```bash
cd data_analysis/data_analysis_visualization
Rscript run_all.R
```

Generated PNGs are written to `figures/` (git-ignored). If `data/all_sty_data.tsv` exists, three additional comparison plots (pSTY vs background STY) are produced automatically.

| Figure | Title |
|--------|-------|
| 01 | Evolutionary conservation of P-sites (histogram) |
| 02 | Exact conservation by residue type (pS / pT / pY) |
| 03 | Ordered vs disordered P-sites (bar chart) |
| 04 | Disorder score by residue type |
| 05 | Exposed vs buried P-sites (bar chart) |
| 06 | Surface exposure by residue type |
| 07 | AlphaFold2 pLDDT scores (histogram with confidence zones) |
| 08 | pLDDT score by residue type |
| 09 | Conservation in ordered vs disordered P-sites |
| 10 | Conservation vs surface exposure |
| 11 | Disorder score vs surface exposure |
| 12* | Disorder score — pSTY vs all S/T/Y background |
| 13* | pLDDT — pSTY vs all S/T/Y background |
| 14* | Surface exposure — pSTY vs all S/T/Y background |

\* Requires `data/all_sty_data.tsv` (generated by `make process`).
